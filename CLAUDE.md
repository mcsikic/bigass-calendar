...
**Self-heal, two layers.** `scripts/sync.sh` runs **every hour on the hour** (`com.mcsk.bigass-sync`): publishes if the Mac is ahead of the site, silent otherwise — it writes to `/tmp/bigass-sync.log` only when it actually publishes or fails, so a quiet log means in sync. `scripts/morning.sh` (`com.mcsk.bigass-digest`, 08:30) publishes first, then sends the digest. Worst-case staleness is therefore **≤1 hour**, and does not depend on any session remembering to publish. Both use `StartCalendarInterval`, not `StartInterval` — per `man launchd.plist` the latter's firings are *missed* on sleep while the former runs at wake. `deploy.sh` detects change by hashing the *plaintext* into `data/.deployed.sha256`; comparing ciphertext would commit on every run, because each encryption uses a fresh random salt and IV.
